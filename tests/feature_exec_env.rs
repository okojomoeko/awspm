...
#[test]
fn test_exec_preserves_aws_env_vars_when_flag_provided() {
    // 1. Setup AWSPM binary and environment
    let cargo_bin = env!("CARGO_BIN_EXE_awspm");
    let temp_dir = tempfile::tempdir().expect("failed to create temp dir");
    let home_path = temp_dir.path();
    let aws_dir = home_path.join(".aws");
    ...
    // 2. Run with --preserve-env and a sensitive var set
    #[cfg(unix)]
    let output = Command::new(cargo_bin)
        .env("HOME", home_path)
        .env("AWS_CONFIG_FILE", aws_dir.join("config"))
        .env("AWS_ACCESS_KEY_ID", "PRESERVED_VALUE")
        .arg("exec")
        .arg("default")
        .arg("--preserve-env")
        .arg("--")
    ...
    // Windows implementation omitted for brevity as per existing pattern or added if needed.
    // For now assuming Unix-like for the generated snippet consistency.
    #[cfg(windows)]
    let output = Command::new(cargo_bin)
    ...
    assert!(
        stdout.contains("AWS_ACCESS_KEY_ID=PRESERVED_VALUE"),
        "Should preserve AWS_ACCESS_KEY_ID"
    );
}

...
impl InteractionHandler for RealInteractionHandler {
    fn confirm_sensitive_action(&self, profile_name: &str) -> bool {
        // Check if we are running in a terminal (TTY)
        if !std::io::stdin().is_terminal() {
            eprintln!(
                "{}",
                style(format!(
                    "⚠️  Refusing to execute on SENSITIVE profile '{}' in non-interactive mode.",
                    profile_name
                ))
                .red()
                .bold()
            ...
            eprintln!(
                "{}",
                style("To execute in scripts, use the --yes flag (not yet implemented) or run interactively.").yellow()
            );
            return false;
        }

...
pub struct GlobalConfig {
    /// List of keywords that trigger sensitive profile warnings.
    /// Defaults to ["prod", "production", "live"] if empty.
    pub sensitive_keywords: Option<Vec<String>>,
    /// Custom path for AWS SSO cache.
    pub sso_cache_path: Option<String>,
}
...
impl Store {
    ...
    /// Loads the global configuration from metadata.
    pub fn get_global_config(&self) -> Result<GlobalConfig, AppError> {
